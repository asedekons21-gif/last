...
export async function updateSession(request: NextRequest) {
  ...
  if (request.nextUrl.origin.includes("localhost") || request.nextUrl.origin.includes("127.0.0.1")) {
    supabaseResponse.headers.set("Access-Control-Allow-Origin", request.nextUrl.origin)
    supabaseResponse.headers.set("Access-Control-Allow-Credentials", "true")
    supabaseResponse.headers.set("Access-Control-Allow-Methods", "GET, POST, PUT, DELETE, OPTIONS")
    supabaseResponse.headers.set("Access-Control-Allow-Headers", "Content-Type, Authorization, Cookie")
  }
